...
# -*- coding: utf-8 -*-
"""
This module contains a simple graphical user interface for Land Bidding Process.
...
from manager import BiddingGameManager, AiPlayerInterface, Player, InvalidMoveError, AiTimeoutError
from utilities import get_possible_moves, get_score

class BiddingGui(object):
...
def main(argv):
    ...
    try:
        ...
        print('bidding_gui.py -d <dimension> [-a <agentA> -b <agentB> -l <depth-limit> -c -o -m]')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('bidding_gui.py -d <dimension> -a <agentA> [-b <agentB> -l <depth-limit> -c -o]')
            sys.exit()
    ...
    game = BiddingGameManager(size)
    gui = BiddingGui(game, p1, p2)
    gui.run()
